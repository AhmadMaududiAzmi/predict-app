...
class GrafikController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0)
        {
            ...
            {
                $end_date = Carbon::parse($end_date)->addDays($request->next_predict)->format('Y-m-d');
            }
            ...
            if($check)
            {
                ...
                $url = 'http://127.0.0.1:8008/api/v1/predict?komoditas_id='.$request->komoditas.'&pasar_id='.$request->pasar.'&start_date='.$start_date.'&end_date='.$end_date.'';
        ...
        return view('grafik.index', compact('pagename', 'comodities', 'markets','data','request','pasarSelectedValue','comoditiesSelectedValue','start_date','end_date'));
    }
